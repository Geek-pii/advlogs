...
<ul class="nav nav-tabs" role="tablist">
  @if(!empty($default_tabs))
    @foreach($default_tabs as $key => $value)
      <li role="presentation" class="nav-item {{ $key === 0 ? 'active': null }}">
        <a href="#{{ $value['id'] }}" class="nav-link" data-toggle="tab" aria-expanded="false">
  ...
  @foreach($composer_locales as $key => $locale)
        <?php $key_trans = $key ?>
    <li role="presentation" class="nav-item {{ empty($default_tabs) && $key === $composer_locale ? 'active' : null }}">
      <a href="#{{ !empty($tab_id) ? $tab_id : 'trans' }}_{{ $key }}" data-toggle="tab" aria-expanded="false" class="nav-link">
...
<div class="tab-content p-t-20">
  ...
  @foreach($composer_locales as $key => $value)
    <div role="tabpanel"
         class="tab-pane fade {{ empty($default_tabs) && $key === $composer_locale ? 'active in' : null }}"
         id="{{ !empty($tab_id) ? $tab_id : 'trans' }}_{{ $key }}">
        ...
        <div class="row">
          <div class="col-sm-12">
            @foreach($form_fields as $field)
              ...
              @if($field["type"] === "photo_translation")
                <div class="row">
                  <div class="col-md-4 col-md-offset-4">
                    <div class="font-bold col-blue">{!! trans("{$translation_file}.form.{$field["name"]}") !!}</div>
              ...
              @elseif($field["type"] === "photo-translation-service")
                <div class="photo-translation-service row" style="display: none">
                  <div class="col-md-4 col-md-offset-4">
                    <div class="font-bold col-blue">{!! trans("{$translation_file}.form.{$field["name"]}") !!}</div>
                    <div class="form-group">
                      @component('admin.layouts.components.upload_photo', [
                ...
                <div class="form-group form-float">
                  ...
                  <div class="form-line">
                    ...
                    @if($field["name"] === 'url' || $field["name"] === 'guide_file')
                      <button type="button" class="btn btn-primary btn_select_a_file" data-append="#{!! $input_id !!}"
              ...
              @elseif($field["type"] === "ckeditor")
                <div class="form-group">
                  <div class="font-bold col-blue m-b-5">{!! trans("{$translation_file}.form.{$field["name"]}") !!}</div>
                  <div class="form-line">
              ...
              @elseif($field["type"] === "ckeditor-service")
                <div class="ckeditor-service form-group" style="display: none">
                  <div class="font-bold col-blue m-b-5">{!! trans("{$translation_file}.form.{$field["name"]}") !!}</div>
                  <div class="form-line">
                                    <textarea rows="6" id="{!! $input_id !!}"
                                              name="{!! $input_name !!}">{!! !empty($object_trans) ? $object_trans->{"{$field['name']}:{$key}"} : old("{$key}.{$field["name"]}") !!}</textarea>
      ...
      @if(!empty($tab_seo))
        ...
        <div class="row">
          <div class="col-sm-12">
            <div class="font-bold col-blue">Image</div>
            <div class="form-group">
              @component('admin.layouts.components.upload_photo', [
                  'image' => $metadata->{"image:{$key}"} ?? '',
                  'name' => 'metadata['. $key .'][image]',
              ])
              @endcomponent
            </div>
            <div class="form-group form-float">
              <div class="font-bold col-blue">{!! trans("admin_seo.form.title") !!}</div>
              <div class="form-line">
            ...
            <div class="form-group form-float">
              <div class="font-bold col-blue">{!! trans("admin_seo.form.description") !!}</div>
              <div class="form-line">
            ...
            <div class="form-group form-float">
              <div class="font-bold col-blue">{!! trans("admin_seo.form.key_word") !!}</div>
              <div class="form-line">
                            <textarea rows="3" class="form-control no-resize"
...
@if(!empty($is_ajax_request))
  @if(!empty($form_plugins))
    ...
    @if(in_array('ace', $form_plugins))
      <script src="{{ asset('assets/plugins/ace-builds/src-noconflict/ace.js') }}" type="text/javascript"
