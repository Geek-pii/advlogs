@foreach($composer_locales as $key => $value)
  <div role="tabpanel" class="tab-pane fade {{ $key === $default_tab ? 'active in' : null }}"
        ...
        <div class="col-sm-12">
          @foreach($form_fields as $field)
            @php
              ...
              $input_id = Str::slug($input_name, '');// !empty($field['default_name']) ? str_replace('LOCALE', $key, $field['default_name']) : "{$key}_{$field["name"]}";
            @endphp
...
@push('add_script')
  @if(!empty($form_fields))
    ...
    @if($collection->contains('type', 'ckeditor'))
      ...
      <script src="{{ asset('assets/plugins/ckfinder/ckfinder.js') }}" type="text/javascript"></script>
    @endif
