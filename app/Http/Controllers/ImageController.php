...
class ImageController extends Controller
{
    ...
    public function getImageThumbnail($path, $width = null, $height = null, $type = 'fit')
    {
        ...
        $thumb_path = config('definitions.thumb_path');
        ...
        if (File::exists(public_path("{$thumb_path}/thumbs/" . "{$width}x{$height}/" . $path))) {
            return url("{$thumb_path}/thumbs/" . "{$width}x{$height}/" . $path);
        }
        ...
        if (in_array($contentType, $allowedMimeTypes)) { //Checks if is an image
            ...
            if (!File::exists(public_path("{$thumb_path}/thumbs/" . "{$width}x{$height}/" . $dir_path))) {
                File::makeDirectory(public_path("{$thumb_path}/thumbs/" . "{$width}x{$height}/" . $dir_path), 0775, true);
            }

            //Save the thumbnail
            $image->save(public_path("{$thumb_path}/thumbs/" . "{$width}x{$height}/" . $path));
